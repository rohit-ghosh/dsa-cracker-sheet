...
bool pairSortByFirstElementComparator(pair<ll,ll> &a, pair<ll,ll> &b){
...
int main(){
  ...
  while(T--){
    ...
    fl(i,0,n-1,1){
        in(intervals[i].first);
        in(intervals[i].second);
    }
    sort(intervals.begin(), intervals.end());
    
    vector<pair<ll,ll>> merged_intervals;
    ll i = 0;
    while(i<n){
        ll start = intervals[i].first, end = intervals[i].second;
        ll j = i+1;
        while(j<n && intervals[j].first >= intervals[i].first && intervals[j].first <= intervals[i].second){
            end = max(end, intervals[j].second);
            j++;
        }
        merged_intervals.push_back(make_pair(start,end));
        i = j;
    }
    cout<<"merged_intervals: ";
    line;
    for(auto merged_interval: merged_intervals){
        cout<<merged_interval.first<<","<<merged_interval.second;
        line;
    }
  }
}