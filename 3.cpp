...
ll getKthSmallestInArray(vll &arr, ll n, ll k){
    priority_queue<ll> kSmallestElementsMaxHeap;
    fl(i, 0, k-1, 1){
        kSmallestElementsMaxHeap.push(arr[i]);
    }
    fl(i, k, n-1, 1){
        ll currentMaxElementInHeap = kSmallestElementsMaxHeap.top();
        if(currentMaxElementInHeap > arr[i]){
            kSmallestElementsMaxHeap.pop();
            kSmallestElementsMaxHeap.push(arr[i]);
            cout<<"Removed "<<currentMaxElementInHeap<< " from heap and new max element in heap is " << kSmallestElementsMaxHeap.top();
            line;
        }
    }
    return kSmallestElementsMaxHeap.top();
}
...
int main(){
  ...
  while(T--){
    ...
    ll kthSmallest = getKthSmallestInArray(arr, n, k);
    dbg(kthSmallest);
  }
}