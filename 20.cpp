...
void changeValueAndShiftArrayElements(vll &a, ll n, ll newIndex, ll oldIndex){
    ll prevValue = a[newIndex];
    if(newIndex < oldIndex){
        fl(i,newIndex+1,oldIndex,1){
            ll temp = a[i];
            a[i] = prevValue;
            prevValue = temp;
        }
...
int main(){
  ...
  while(T--){
    ...
    fl(i,0,n-1,1){
        if(a[i] < 0){
            negativeWindow += 2;
            changeValueAndShiftArrayElements(a,n,negativeWindow,i);
            if(negativeWindow > i) i = negativeWindow + 1;
        }
    }
