...
int main(){
  ...
  while(T--){
    ...
    in_ds(arr2,n2);
    umll freq;
    fl(i, 0, min(n1,n2)-1, 1){
      freq[arr1[i]]++;
      freq[arr2[i]]++;
    }
    fl(i, min(n1,n2), max(n1,n2)-1, 1){
      if(n1 > n2){
        freq[arr1[i]]++;
      }
      else{
    ...
    vll intersectionOfArrays;
    cout<<"Union: ";
    for(auto pair: freq){
      cout<<pair.first<<" ";
      if(pair.second > 1) intersectionOfArrays.push_back(pair.first);
    }
    line;
    cout<<"Intersection: ";
    dbg_ds(intersectionOfArrays);
  }
}