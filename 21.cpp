...
Output: false
*/
bool containsSubarrayWithSumZero(vll &a, ll n){
    if(n) {
        unordered_map<ll,vector<ll>> prefixSumToIndicesMap;
        ll prefixSum = a[0];
        prefixSumToIndicesMap[prefixSum].push_back(0);
        fl(i,1,n-1,1){
            prefixSum += a[i];
            if(prefixSumToIndicesMap.find(prefixSum) != prefixSumToIndicesMap.end()) return true;
            prefixSumToIndicesMap[prefixSum].push_back(i);
        }
        return false;
    }
...
int main(){
  ...
  while(T--){
    ...
    dbg(containsSubarrayWithSumZero(a,n));
  }
}