...
int main(){
  ...
  while(T--){
    ...
    in_ds(a, n);
    ll i = 0;
    ll ans = a[i], max_sum_of_subarray_ending_at_i = a[i];
    while(++i < n){
        max_sum_of_subarray_ending_at_i = max(max_sum_of_subarray_ending_at_i + a[i], a[i]);
        ans = max(ans, max_sum_of_subarray_ending_at_i);
    }
    dbg(ans);
  }
}